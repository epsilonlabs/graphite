...
public class DerivedObjectUtility {
	...
	public static void setDerivedObjects(XtextEObject object) {
		for (DerivedObjectProperties derivedObjectProperties: object.getDerivedObjectsProperties().values()) {
			setDeriveObject(derivedObjectProperties);
		}
	}
	...
	public static void setDeriveObject(DerivedObjectProperties derivedObjectProperties, String parsedString, Object derivedObject) {
		if(!derivedObjectProperties.getLock().isLocked() && (!Objects.equals(parsedString, derivedObjectProperties.getLastParsedString()) || derivedObjectProperties.isInvalid())) {
			...
			if (!derivedObjectProperties.isInvalid()) {
				newDerivedObject = !derivedObjectProperties.isTransformed() ? newDerivedObject : getTransformedObject(derivedObjectProperties, newDerivedObject);
	...
	public static void setDerivedObject(DerivedObjectProperties derivedObjectProperties, EObject currentDerivedObject, EObject newDerivedObject) {
		...
		if (isIdenticalDerivedObject(derivedObjectProperties, newDerivedObject, currentDerivedObject)) {
	...
	private static List<EObject> getTransformedObject(DerivedObjectProperties derivedObjectProperties, List<EObject> objects) {
		BasicEList<EObject> transformedObjects = new BasicEList<EObject>();
		for (EObject object : objects) {
			Object transformedObject = getTransformedObject(derivedObjectProperties, object);
			if (transformedObject instanceof List) {
				transformedObjects.addAll((List)transformedObject);
			}
			else {
				transformedObjects.add((EObject)transformedObject);
			}
		}
		return transformedObjects;
	}
	
	private static Object getTransformedObject(DerivedObjectProperties derivedObjectProperties, EObject object) {
		return derivedObjectProperties.getTransformationService().transform(object);
	}
	
	private static Object getTransformedObject(DerivedObjectProperties derivedObjectProperties, Object object) {
		if (object instanceof List) {
			return getTransformedObject(derivedObjectProperties, (List) object);
		}
		return getTransformedObject(derivedObjectProperties, (EObject) object);
	}
	
	public static Object deserializeObject(DerivedObjectProperties derivedObjectProperties, String parsedString) {
		derivedObjectProperties.getDerivedObjectGetter().setAccessible(true);
		Resource derivedResource = XtextParser.deserializeString(derivedObjectProperties, parsedString);
	...
	public static void serializeObject(DerivedObjectProperties derivedObjectProperties) {
		if(!derivedObjectProperties.getLock().isLocked()) {
			...
			try {
				...
				String serializationResult = XtextParser.serializeObject((EObject)containerObject, derivedObjectProperties.getGrammar());
